...
def calculate(
    ...
    if commissioning_date is not None:
        ...
        # Deflate each historical month's savings to today's PLN
        real_sav = sum(
            _msav(r) / (1.0 + inflation) ** (
                ((today.year - r.year) * 12 + (today.month - r.month)) / 12.0
            )
            for r in complete if _msav(r) > 0
        )
        real_total_savings = round(real_sav, 2)
        real_subsidy = subsidy / (1.0 + inflation) ** years_elapsed
